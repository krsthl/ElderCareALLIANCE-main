		...
		if (isset($_POST['UpdateCredentials'])) {
			...
			if ($oldUser == $res['username']) {
				...
				if ($oldPass == $res['password']) {
					if ($newPass == "" && $rePass == "") {
						$result = mysqli_query($conn, "UPDATE staff SET username = '$user' WHERE username='$oldUser'");

						if ($result) {
							echo '<script>alert("New Credentials Updated. Logging Out")</script>';
							echo "<script>window.location.href='/ElderCareALLIANCE/admin/index.php'</script>";
						} else {
					...
					// checks if the new pass and and re enter password is the same and performs the query
						...
						$result = mysqli_query($conn, "UPDATE staff SET username = '$user', password = '$rePass' WHERE username='$oldUser'");
